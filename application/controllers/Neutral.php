...
class Neutral extends CI_Controller
{
  ...
  public function __construct()
    ...
    else
    {
      ...
      $this->data['business'] = $this->Neutrals->get_business_details($this->user_id);
  ...
  {
    $this->data['home_title'] = $this->Neutrals->get_home_title($this->user_id);
    $this->data['home_description'] = $this->Neutrals->get_home_description($this->user_id);
    $this->data['home_bg'] = $this->Neutrals->get_home_background_image($this->user_id);
  ...
  public function save_home()
  {
    ...
    if ($this->form_validation->run() == TRUE)
    {
      if(!empty($_FILES['picture']['name'])){ //If may laman na image
        $query = $this->db->query("select * from contents where user_id = '$this->user_id' and meta_key = 'home_background_image'");
        if ($query->num_rows() > 0)
        {
      ...
      if (!empty($this->input->post('title')))
      {
        ...
        if ($query->num_rows() > 0)
        {
          $data['home_title'] = $this->Neutrals->get_home_title($this->user_id);
          $Home_Title = [
      ...
      if (!empty($this->input->post('description')))
      {
        ...
        if ($query->num_rows() > 0)
        {
          $data['home_description'] = $this->Neutrals->get_home_description($this->user_id);
          $Home_Description = [
  ...
  {
    $this->data['about_bg'] = $this->Neutrals->get_about_background_image($this->user_id);
    $this->data['about_title'] = $this->Neutrals->get_about_title($this->user_id);
    $this->data['about_description'] = $this->Neutrals->get_about_description($this->user_id);
  ...
  public function save_about()
  {
    ...
    {
      if(!empty($_FILES['picture']['name'])){ //If may laman na image
        $query = $this->db->query("select * from contents where user_id = '$this->user_id' and meta_key = 'about_background_image'");
        if ($query->num_rows() > 0)
        {
      ...
      if (!empty($this->input->post('title')))
      {
        $query = $this->db->query("select * from contents where user_id = '$this->user_id' and meta_key = 'about_title'");
        if ($query->num_rows() > 0) {

          $data['about_title'] = $this->Neutrals->get_about_title($this->user_id);
          $Title = [
            'value' => $this->input->post('title')
          ];
      ...
      if (!empty($this->input->post('description')))
      {
        ...
        if ($query->num_rows() > 0) {
          ...
          $data['about_description'] = $this->Neutrals->get_about_description($this->user_id);
          $About_Description = [
            'value' => $this->input->post('description')
          ];
  ...
  {
    $this->data['gallery_images'] = $this->Neutrals->get_gallery_images($this->user_id);
  ...
  {
    $this->data['facebook'] = $this->Neutrals->get_facebook_url($this->user_id);
    $this->data['instagram'] = $this->Neutrals->get_instagram_url($this->user_id);
    $this->data['twitter'] = $this->Neutrals->get_twitter_url($this->user_id);
  ...
  public function save_template()
  {
    ...
    if ($this->Neutrals->update_user_template($Template,$this->user_id)) //KAPAG SUCCESSFULLY NAGUPDATE ANG TEMPLATE
    {
