          ...
          <div class="box-body">
            ...
            <div class="col-lg-6">
              ...
              <div class="form-group">
                ...
                <select class="form-control" name="contract" id="contract">
                  <?php if ($ad->contract == 'Month'): ?>
                    <option value="<?php echo $ad->contract?>" selected>1 month</option>
                    <option value="Quarter">Quarter</option>
                    <option value="Half">Half</option>
                    <option value="Year">1 year</option>
                  <?php elseif ($ad->contract == 'Quarter'): ?>
                    <option value="<?php echo $ad->contract?>" selected>Quarter</option>
                    <option value="Month">1 month</option>
                    <option value="Half">Half</option>
                    <option value="Year">1 year</option>
                  <?php elseif ($ad->contract == 'Half'): ?>
                    <option value="<?php echo $ad->contract?>" selected>Half</option>
                    <option value="Month">1 month</option>
                    <option value="Quarter">Quarter</option>
                    <option value="Year">1 year</option>
                  <?php elseif ($ad->contract == 'Year'): ?>
                    <option value="<?php echo $ad->contract?>" selected>1 year</option>
                    <option value="Month">1 month</option>
                    <option value="Quarter">Quarter</option>
                    <option value="Half">Half</option>
                  <?php endif; ?>
              ...
              <div class="form-inline">
                <div class="form-group">
                  <label>Period*</label>
                  <input type="date" name="start" value="<?php echo $ad->start_date ?>" id="start">
                  <span style="color:red" class="help-block"><?php echo form_error('start'); ?></span>
                </div>
                <div class="form-group">
                  <input type="date" name="end" value="<?php if (!empty($ad->end_date)): ?><?php echo $ad->end_date ?><?php endif; ?>" id="end">
                  <span style="color:red" class="help-block"><?php echo form_error('end'); ?></span>
                </div>
              </div>
              <div class="form-group">
                <label>Business ID*</label>
                <input type="text" name="business_id" class="form-control" value="<?php echo $ad->business_id ?>" placeholder="Enter business ID" id="business_id">
                <span style="color:red" class="help-block"><?php echo form_error('business_id'); ?></span>
              </div>
              <div class="form-group">
                <label>Title*</label>
                <input type="text" name="title" class="form-control" value="<?php echo $ad->title ?>" placeholder="Enter advertisement title" id="title">
                <span style="color:red" class="help-block"><?php echo form_error('title'); ?></span>
              </div>
              <div class="form-group">
                <label>Subtext*</label>
                <input type="text" name="subtext" class="form-control" value="<?php echo $ad->subtext ?>" placeholder="Enter advertisement subtext" id="subtext">
                <span style="color:red" class="help-block"><?php echo form_error('subtext'); ?></span>
              </div>
              <div class="form-group input-width center-block">
                <label>Description*</label>
                <textarea class="form-control" name="description" placeholder="Tell something about the advertisement here" rows="10" id="description"><?php echo $ad->description ?></textarea>
...
$('.cropped_image').on('click', function (ev) {
  if ($('#images').val()=='') {
    ...
    var description = $('#description').val();
		$.ajax({
			url: "<?php echo base_url() ?>Admin/update_ad/<?php echo $ad->advertisement_id?>",
			type: "POST",
